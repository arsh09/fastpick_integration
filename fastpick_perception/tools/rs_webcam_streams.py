...
from tools.rs_insertPointCloudSeg import MapManager, callback, pointcloud_from_depth, rs_callback, rs_callback_o3d_vtx
...
class MsgManager(object):
    def __init__(self):
        ...
        # self.obj_data = None
        # self.ctl_data = None
        # self.caminfo_data = CameraInfo()
        self.MapObj = MapManager()
        # self.caminfo_data.K = np.array(self.caminfo_data.K).reshape(3, 3)
    ...
    def depth_callback(self, msg):
        ...
        depth_image[depth_image > 0.5] = 0.0
    ...
    def caminfo_callback(self, msg):
        ...
        """
        rospy.loginfo('cam info fx :  %f',self.MapObj.fx)
        rospy.loginfo('cam info fy :  %f',self.MapObj.fy)
        rospy.loginfo('cam info ppx :  %f',self.MapObj.ppx)
        rospy.loginfo('cam info ppy :  %f',self.MapObj.ppy)"""

        """        
        rospy.loginfo('cam info width :  %f',self.MapObj.camera_info.width)
        rospy.loginfo('cam info height :  %f',self.MapObj.camera_info.height)

        rospy.loginfo('cam info fx :  %f',self.MapObj.camera_info.K[0])
        rospy.loginfo('cam info fy :  %f',self.MapObj.camera_info.K[4])
        rospy.loginfo('cam info ppx :  %f',self.MapObj.camera_info.K[2])
        rospy.loginfo('cam info ppy :  %f',self.MapObj.camera_info.K[5])
        # self.caminfo_data.K[0, 0] = self.depth_intrin_alligned.fx
        # self.caminfo_data.K[1, 1] = self.depth_intrin_alligned.fy
        # self.caminfo_data.K[0, 2] = self.depth_intrin_alligned.ppx
        # self.caminfo_data.K[1, 2] = self.depth_intrin_alligned.ppy
        # rospy.loginfo('cam info fx : %f',self.caminfo_data.K[0,2])
    ...
    def process_message(self):
        ...
        occupied, empty = self.MapObj.rs_callback_octomap()
        ...
        scale_per = 0.5
        img = cv2.merge(
            (self.MapObj.depth_image, self.MapObj.depth_image, self.MapObj.depth_image))
...
class StreamManager(object):
    ...
    def rs_init(self):
        ...
        self.cam_info.height = self.depth_intrin_alligned.height
        self.cam_info.width = self.depth_intrin_alligned.width
        ...
        self.cam_info.K[0, 0] = self.depth_intrin_alligned.fx
        self.cam_info.K[1, 1] = self.depth_intrin_alligned.fy
        self.cam_info.K[0, 2] = self.depth_intrin_alligned.ppx
        self.cam_info.K[1, 2] = self.depth_intrin_alligned.ppy
    ...
    try:
        ...
        while True:
            ...
            occupied, empty  = rs_callback(data)
            # Show images
