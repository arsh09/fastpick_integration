...
dest_dir = '/data/localdrive/strawberry_multi_cam/detectron/'

for rgb_file in pathlib.Path(img_dir).rglob('*.png'):
    ...
    pred_box, pred_mask = outputs.pred_boxes.tensor.numpy(), outputs.pred_masks.numpy().reshape(480, 640)

    im_seg = im * np.dstack((pred_mask, pred_mask, pred_mask))
    # print('boxes shape: ')
    # if boxes.shape == (1,4):
    #     predict_weight(im)
    ...
    # cv2.imshow('inference', out.get_image()[:, :, ::-1])
    bbox_mask = {'pred_box': pred_box, 'pred_mask: ': pred_mask}
    np.save(dest_dir + rgb_file.name.strip('.png'), bbox_mask)
    # cv2.imshow('inference', im_seg)
    # key = cv2.waitKey(0)
    # if key == ord('q'):
    #     break
    # else:
